...
machine failures in social-networking applications have turned down the 
...
leader controller to handle all jobs. If this controller fails, \paxos elects 
...
systems only use \paxos to elect a leader, not to agree on inputs (\ie, job 
requests).
...
To avoid the two aforementioned issues, \xxx chooses to integrate \paxos in the 
...
\xxx ensures that minor failures in a job does not affect its other replicated 
jobs to compute results efficiently. By leveraging \mesos's resource allocation 
and isolation features, \xxx can run diverse applications on the same physical 
machine, reducing much of the hardware cost in traditional replication 
techniques.
...
To emulate a typical social-networking deployment, we ran this protocol with 
\redis, a popular key-value store that Twitter uses, and we ran \redis's own 
benchmark workload. Evaluation showed that, compared to \redis's unreplicated 
execution, \xxx incurred merely a \tputoverhead\% overhead in throughput and 
\latencyoverhead\% in response time. This protocol was 40.1X faster than 
a \zookeeper SMR protocol~\cite{calvin:sigmod12} which runs on TCP/IP.
