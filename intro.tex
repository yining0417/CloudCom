...
then minor replicas can just transparently run an analysis tool on each and 
majority replicas run actual executions to process inputs fast.
...
We be have conducted \xxx's feasibility study in Linux. To leverage \smr, \xxx 
incoporates a \paxos protocol implementation in \repbox, a transparent \smr 
system for general server applications. To leverage \dmt, \xxx incoporates 
\parrot, a \dmt runtime system that has shown to run fast on a wide range of 
...
In the remaining of this paper, \S\ref{sec:background} introduces the background 
of \smr and \dmt, \S\ref{sec:overview} presents an overview \xxx's design, 
