...
to an anlysis tool. Traditional analysis frameworks take a ``fully-coupled" 
approach: a framework inlines an analysis with the execution, then the 
analysis can inspect all or most execution states (\eg, Valgrind, DynamoRio, 
Pin, and LIFT). However, this approach can easily let the analysis slow down 
the actual execution. To improve performance, leveraging a fact that many 
analyses can be done asynchronously with the actual execution, some recent 
frameworks take a second ``partially-decoupled" approach: only the execution 
states critical to the analysis (\eg, effective memory addresses and thread 
interleavings) are exposed to the analysis, then the actual execution can be 
less perturbed by the analysis. Existing analysis frameworks have embraced 
numerous analysis tools and have greatly improved software quality (\eg, 
detecting data races in multithreaded programs).
