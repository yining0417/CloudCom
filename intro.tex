...
approach may easily let the analysis slow down the actual execution, some 
recent frameworks take a second ``partially-decoupled" approach: only a 
necessary subset of execution states (\eg, effective memory addresses and 
thread interleavings) are exposed to the analysis, then the actual execution 
can be less perturbed by the analysis. These analysis frameworks have been 
widely studied and greatly improved software quality (\eg, detecting data races 
...
actual execution, so whenever an analysis is slow, the analysis is slowed down 
(\eg, 30X to 200X slow down for race detection tools~\cite{tsan}). Recent 
frameworks that take the ``partially-decoupled" approach has shown about 4X 
speedup over the traditional frameworks because they transfer fewer execution 
states, however, the overall slowdown of these frameworks are still prohibitive 
because they still need to the amount of transfered execution states (\eg, 
effective memory addresses and thread interleavings) to analysis tools are 
still enormous.
...
these frameworks heavily orchest the analysis tools as well as the transfered 
execution states, which causes the analysis tools to be heavily reconstructed. 
For example, a recent framework for race detection tools leverages the 
record-replay technique to offload the analysis from the actual execution, 
but this framework requires significantly refactoring race detection tools into 
several phases, which is largely different from a typical one (Note that an 
analysis tool itself is usually extremely difficult to implement and maintain.)
...
memory approach to hold analysis results of the actual execution's memory, and 
different analyses may require different copies of shadow memory. It is not 
trivial for these frameworks to support multiple copies of shadow memory. It is 
not trival for ``paritall-decoupled" frameworks to support multiple types of 
analysis tools at the same time either, because they heavily orchest the 
transfered execution states for each type of analysis.


A key common reason for these three problems in existing analysis frameworks is 
that they only have one actual execution, so they have to either fully couples 
the analysis with the actual execution, or partially decouples the analysis 
from the execution and then explicitly transfer a sub set of execution states 
to the analysis.
...
This paper presents \xxx, an efficient, transparent dynamic program analysis 
framework. Our key insight is that we can actually construct multiple 
equivalent executions by levaraging a technique called state machine replicaion 
(or \smr). It models a program as a deterministic state machine, where states 
are important program data and the transitions are deterministic
...
executions, because today's applications are largely nondeterministic: the 
thread interleavings (or emph{schedules}) in different replica can easily lead 
to divergent execution states and break \smr. To address this problem, \xxx 
leverages deterministic multithreading (or \dmt), a recent advanced threading 
technique that can efficiently enforce that all inter-thread communications 
happen in the same order, so that different replicas in \smr run into the same 
schedules and progress with the same execution states. Recent 
study~\cite{parrot:sosp13} has shown that \dmt runs fast on a wide range of 108 
popular multithreaded programs on 24-core machines. In sum, by leveraging \smr 
and \dmt, we can construct equivalent execution in replicas, so that \xxx can run 
multiple anlyses in minor replicas transparently, while the majority of the 
replicas can still reach consensus and process inputs fast.
...
The main contribution of \xxx is the idea of constructing equavalent executions 
to fully decoupe execution and analysis. Leveraging the replication 
...
the performance considerations (\ref{sec:discuss}).
