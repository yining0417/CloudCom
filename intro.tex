...
data, cluster management systems (\eg, \mesos~\cite{mesos:nsdi11}) are 
...
tends to be highly-available during the stock operation hours, and adding a few 
...
Specifically, to make the systems themselves highly-available, existing systems 
typically replicate their controller component which accepts tasks, allocate 
computational resources, and (re)schedule tasks on available resources. 
...
systems have to reschedule the tasks, leaving an arbitrary unavailable time 
...
protocol with \mesos~\cite{mesos:nsdi11}, a widely used cluster management 
...
the physical hardware cost in typical \paxos.
...
\paxos replication architecture for simplicity and generality.
...
merely \latencyoverhead\% overhead on response time and \tputoverhead\% overhead 
on throughput. \xxx's SMR protocol was 40.1X faster than \zookeeper's SMR 
protocol~\cite{calvin:sigmod12}.
